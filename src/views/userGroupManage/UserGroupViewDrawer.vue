<template>
	<el-drawer v-model="drawerVisible" :destroy-on-close="true" size="500px" :title="`${drawerData.title}用户组`">
		<el-form
			ref="ruleFormRef"
			:rules="rules"
		...
		>
			...
			<el-form-item label="用户组编码" prop="userGroupCode">
				<el-input v-model="drawerData.rowData!.userGroupCode" placeholder="请填写用户组编码" clearable></el-input>
			</el-form-item>
		</el-form>
		<template #footer>
...
const rules = reactive({
	userGroupName: [{ required: true, message: "请填写用户组名称", trigger: "change" }],
	userGroupCode: [{ required: true, message: "请填写用户组编码", trigger: "change" }]
});
